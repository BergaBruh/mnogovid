...
SSH alias or explicit `user@host` (no local SSH config read). Obtain explicit
connection consent before `system_remote_prepare` (`approveConnection=true`). Require separate root, network,
